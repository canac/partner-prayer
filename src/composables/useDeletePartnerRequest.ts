...
export default function useDeletePartnerRequest(): {
  ...
  async function deletePartnerRequest(deletingRequest: PartnerRequest): Promise<void> {
    await mutate({
      input: {
      ...
      update(cache) {
        ...
        if (partner) {
          cache.writeFragment({
            ...
            fragment,
            data: {
              requests: partner.requests.filter((request) => request._id !== deletingRequest._id),
            },
          });
        }
      },
